...
- [概念剖析-flask数据库操作](#概念剖析-flask数据库操作)
  ...
  - [使用 `Flask-SQLAlchemy` 管理数据库](#使用-flask-sqlalchemy-管理数据库)
    ...
    - [集成 Python shell](#集成-python-shell)
    - [数据库表的修改](#数据库表的修改)
- [附录](#附录)
...
##### 集成 Python shell 
> 可以通过Shell 来调试模块，据说，[文档](http://www.pythondoc.com/flask/shell.html)
> 每次启动 shell 都要导入数据库实例和模型，通过设置让 `Flask-Script`的 `shell` 自动导入特定的对象
...
> 开发过程中有时候需要修改数据库模型，并更新到数据库，需要用到数据库迁移框架，推荐 `Alembic`，及 `Flask - Migrate`扩展（封装了`Alembic`），所有的操作通过 `Flask-Script`完成
...
```
* 创建迁移仓库 `>>> python hello.py db init`，会创建 `migrations`文件夹，放置所有的迁移脚本，**迁移仓库中的要进行版本控制**
* 数据库的迁移用 **迁移脚本** 表示，脚本中有两个函数 `upgrade()` 和 `downgrade()` 前者应用修改，后者删除修改，数据库可以重置到修改历史的任一点。
    * `revision` 手动创建迁移，`upgrade()` 和 `downgrade()` 都是空的，开发者需要使用 `Operations`对象指令实现具体操作
    * `migrate` 命令自动创建，会根据模型定义和数据库当前状态之间的差异自动生成 `upgrade()` 和 `downgrade()` 的内容
...
**PROBELM** 前文说道，`迁移`而不是`删除表格，重新创建表格`能够保存数据库中的数据，那么如何指定新列于旧列的对应关系，及其他更精细的操作？ 
